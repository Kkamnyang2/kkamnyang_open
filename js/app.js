...
function openEmojiPicker(targetInputId) {
    ...
    document.getElementById('emoji-picker-dialog').style.display = 'flex';
    
    // 기본 카테고리 표시 (동물)
    showEmojiCategory('animals');
}
...
function closeEmojiPicker() {
    document.getElementById('emoji-picker-dialog').style.display = 'none';
...
function showEmojiCategory(category) {
    ...
    grid.innerHTML = emojis.map(emoji => `
        <button onclick="selectEmoji('${emoji}')" 
                style="font-size: 28px; padding: 8px; border: none; background: white; border-radius: 4px; cursor: pointer; transition: all 0.2s;"
                onmouseover="this.style.background='#e3f2fd'; this.style.transform='scale(1.2)'"
